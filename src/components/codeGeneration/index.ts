...
export const exportJokersAsMod = async (
  ...
  try {
    ...
    zip.file(metadata.main_file, generateMainLua(jokers, version));
...
const generateMainLua = (jokers: JokerData[], version?: string): string => {
  const versionComment = version ? ` v${version}` : "";

  let header = `-- FILE GENERATED BY JOKER FORGE${versionComment}`;
...
export const exportSingleJoker = (joker: JokerData, version?: string): void => {
  try {
    const jokerCode = generateSingleJokerLua(joker, version);
...
const generateSingleJokerLua = (joker: JokerData, version?: string): string => {
  const versionComment = version ? ` v${version}` : "";

  const header = `-- FILE GENERATED BY JOKER FORGE${versionComment}`;
