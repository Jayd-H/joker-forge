...
export const generateAddModifiersReturn = (
  effect: Effect,
  triggerType: string,
  modprefix: string
): EffectReturn => {
  ...
  if (isNegative || ignoreSlots) {
    slotLimitCode = "local edited_joker = true";
  } else {
    slotLimitCode = `local edited_joker = false
    if #G.jokers.cards + G.GAME.joker_buffer < G.jokers.config.card_limit then
        edited_joker = true
        G.GAME.joker_buffer = G.GAME.joker_buffer + 1`;
  }

  const cardCreationCode = `local joker_card = SMODS.change_base({ ${cardParams.join(
    ", "
  )} })`;
  const editionCode =
    edition !== "none" ? `joker_card:set_edition("${edition}", true)` : ``;
    edition === "remove" ? `joker_card:set_edition(nil, true)` : ``;
    edition === "random" ? `
  ...
  const stickerCode = 
    sticker !== "none" ? `joker_card:set_${sticker}(true)`
    : "";
  ...
  if (isScoring) {
    return {
      ...
      message: customMessage
        ? `"${customMessage}"`
        : `edited_joker and localize('k_plus_joker') or nil`,
      colour: "G.C.BLUE",
    };
  } else {
    return {
